...
impl GloomyDriver {
  ...
  pub fn with_rendering(
    ...
    driver.headless = Some(HeadlessRenderer::new(
      pixel_width,
      pixel_height,
      scale_factor,
    )?);
  ...
  ///
  /// Requires the driver to have been created with
  /// `with_rendering`.
  pub fn render_to_image(
    ...
    let renderer = self
      ...
      .ok_or_else(|| {
        anyhow::anyhow!(
          "no HeadlessRenderer; use with_rendering()"
        )
      })?;
  ...
  ///
  /// Renders the current tree, compares against the stored golden
  /// in `snapshot_dir`, and returns a structured diff report.
    ...
    match widget {
      Widget::Container { id, children, .. } => {
        ...
        for child in children {
          if let Some(found) =
            Self::find_recursive(child, target_id)
          {
            return Some(found);
          }
        }
  ...
  pub fn click(&mut self, id: &str) -> Option<String> {
    ...
    self
      .interaction
      .update_mouse(Vec2::new(center_x, center_y));
